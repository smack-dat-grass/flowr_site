...
url = f"https://api.covid19api.com/live/country/united-states/status/confirmed/date/{yesterday.strftime('%Y-%m-%d')}T00:00:00Z"
...
try:
    ...
    if r.status_code != 200:
        ...
        health_check_metric.message = "Could not load Covid-19 metrics for United States"
        ...
        if delta > attributes['threshold']:
            health_check_metric.message = f"{delta} new Covid-19 Cases Reported in United States in the last 24 hours"
            health_check_metric.icon = 'https://www.iconsdb.com/icons/preview/red/warning-xxl.png'
            health_check_metric.successful = False
        else:
            health_check_metric.message = f"{delta} new Covid-19 Cases Reported in United States in the last 24 hours"
